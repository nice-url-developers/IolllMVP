...
public class MultiTypeAdapter extends RecyclerView.Adapter<ViewHolder> {
    ...
    public MultiTypeAdapter(boolean isMaxBanner) {
        this();
        this.isMaxBanner = isMaxBanner;
    }
    ...
    public final int getItemViewType(int position) {
        Object item = isMaxBanner ? items.get(position % items.size()) : items.get(position);
    ...
    public final void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        onBindViewHolder(holder, isMaxBanner ? position % items.size() :position, Collections.emptyList());
    }
    ...
    int indexInTypesOf(int position, @NonNull Object item) throws BinderNotFoundException {
        ...
        if (index != -1) {
            ...
            return isMaxBanner ? index + linker.index(position % items.size(),item) : index + linker.index(position, item);
        }
     ...
     */
    @SuppressWarnings("unchecked")
    private void registerWithoutChecking(@NonNull Class clazz, @NonNull ItemViewBinder
            binder, @NonNull Linker linker) {
        checkAndRemoveAllTypesIfNeeded(clazz);
